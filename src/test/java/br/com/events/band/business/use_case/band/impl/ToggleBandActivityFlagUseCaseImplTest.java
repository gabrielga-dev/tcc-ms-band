...
class ToggleBandActivityFlagUseCaseImplTest {
    ...
    @InjectMocks
    private ToggleBandActivityFlagUseCaseImpl useCase;
    ...
    @Test
    @DisplayName("execute - when band is found, then toggle status")
    void executeWhenBandIsFoundThenToggleStatus() {
        when(authService.getAuthenticatedPersonUuid()).thenReturn(MockConstants.STRING);
        var mockedBand = mock(BandTable.class);
        doNothing().when(mockedBand).toggleActivity();
        ...
        verify(authService, atMostOnce()).getAuthenticatedPersonUuid();
        verify(findBandCommand, atMostOnce()).byUuidAndOwnerUuid(eq(MockConstants.STRING), eq(MockConstants.STRING));
        verify(mockedBand, atMostOnce()).toggleActivity();
