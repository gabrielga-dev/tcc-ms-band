...
/**
 * Tests for {@link GenerateLineupQuoteRequestPdfCommand}
 *
 * @author gabriel
 */
@ExtendWith(MockitoExtension.class)
class GenerateLineupQuoteRequestPdfCommandTest {

    @InjectMocks
    private GenerateLineupQuoteRequestPdfCommand command;
    ...
    @Test
    @DisplayName("getAcceptedPdfType - when called return lineup")
    void getAcceptedPdfTypeWhenCalledReturnLineup() {
        var returned = command.getAcceptedPdfType();
        Assertions.assertEquals(PdfType.LINEUP, returned);
    }
    ...
    void executeWhenCalledReturnPdfValues() {
        ...
        Assertions.assertEquals("lineup.pdf", returned.getFileName());
        ...
        verify(msEventFeign, times(1)).findProfile(eq(MockConstants.STRING));
